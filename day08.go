...
func day8_part1() {
	...
	for _, c := range lines {
		inst := strings.Split(c, " ")
		mov, _ := strconv.Atoi(inst[1])
		instruction := Instruction{inst[0], mov, 0}
	...
	for {
		...
		if i > len(instructions) {
			fmt.Println("out of bounds > max")
			break
		} else if i < 0 {
			fmt.Println("out of bounds < 0")
			break
		} else {
			if ins.name != "jmp" {
				i++
...
func day8_part2() {
	...
	for passed == false {
		...
		instructions = mutate(instructions, mutation)
		mutation++
		accumulator := 0
		var rancount uint = 0
		i := 0
		for {
			...
			if ins.name == "jmp" && ins.value == 0 || ins.name != "jmp" {
				i++
			}
		}
