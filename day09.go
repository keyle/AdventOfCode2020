...
func init_data() []int {
	contents := getFilesContents("day09.input")
...
func day9_part1() {
	list := init_data()
	find_fist_invalid(list, 25)
}

func find_fist_invalid(list []int, preamble int) int {
...
func find_valid(candidate int, list []int) (bool, int) {
	found := false
	for _, a := range list {
		for _, b := range list {
			if a+b == candidate {
...
func add_to_invalid(list []int, invalid int, start int) (int, int, int) {
	sum := 0
	i := 0
	smallest := 9999999999.0
	largest := 0.0
	for sum < invalid {
		// fmt.Print(list[start+i], " + ")
		sum += list[start+i]
		smallest = math.Min(float64(list[start+i]), float64(smallest))
